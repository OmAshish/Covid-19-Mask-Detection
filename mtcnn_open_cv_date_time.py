...
#detector = MTCNN()
#image = cv2.imread("C:\\Users\\OM MISHRA\\(OM)\\dataset\\training_data\\nomask\\001009.jpg")
...
model = load_model("C:\\Users\\OM MISHRA\\(OM)\\dataset\\mtcnn_face_mask.h5")
...
while True: 
    ...
    cv2.putText(frame,'Om\'s CAM',(480,450), font2, 0.9,(255,255,255),2,cv2.LINE_AA)
...
#            cover = resizeimage.resize_cover(image_file, [160, 160], validate=False)
            ...
            width = 160
            dim = (width, height,3)
#            cover = cv2.resize(image_file, dim, interpolation = cv2.INTER_AREA)
            cover = np.resize(image_file,dim)
